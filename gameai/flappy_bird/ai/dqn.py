...
class ExperienceBuffer():
    ...
    定义经验回报类，完成数据的存储和采样
    """
    def __init__(self, buffer_size=REPLAY_MEMORY) -> None:
        self.buffer = []
...
class DeepQN(object):
    ...
    """
    def __init__(self, lr=1.0e-6, model_file=None) -> None:
        self.gamma = GAMMA
        self.tau = 0.01
        ...
        # 2.4 定义新旧参数的替换操作
        self.update_oldq_op = [oldq.assign((1 - self.tau) * oldq + self.tau * p)
            for p, oldq in zip(self.qe_params, self.qt_params)]
        # 3. 构建损失函数
        # TD target
    ...
    def build_q_net(self, obs, scope, trainable):
        with tf.variable_scope(scope):
            h_conv1 = tf.layers.conv2d(inputs=obs, filters=32, kernel_size=[8, 8],
                strides=4, padding='same', activation=tf.nn.relu,
                kernel_initializer=tf.random_normal_initializer(mean=0, stddev=0.01),
                bias_initializer=tf.constant_initializer(0.01),
                trainable=trainable)
            h_pool1 = tf.layers.max_pooling2d(
                h_conv1, pool_size=[2, 2], strides=2, padding='SAME')
            h_conv2 = tf.layers.conv2d(inputs=h_pool1, filters=64, kernel_size=[4, 4],
                strides=2, padding='same', activation=tf.nn.relu,
                kernel_initializer=tf.random_normal_initializer(mean=0, stddev=0.01),
                bias_initializer=tf.constant_initializer(0.01),
                trainable=trainable)
            h_conv3 = tf.layers.conv2d(inputs=h_conv2, filters=64, kernel_size=[3, 3],
                strides=2, padding='same', activation=tf.nn.relu,
                kernel_initializer=tf.random_normal_initializer(mean=0, stddev=0.01),
                bias_initializer=tf.constant_initializer(0.01),
                trainable=trainable)
            h_conv3_flat = tf.reshape(h_conv3, [-1, 576])
            # 第一个全连接层
            h_fc1 = tf.layers.dense(inputs=h_conv3_flat, units=512, activation=tf.nn.relu,
                kernel_initializer=tf.random_normal_initializer(mean=0, stddev=0.01),
                bias_initializer=tf.constant_initializer(0.01),
                trainable=trainable)
            # 读出层，没有激活函数
            qout = tf.layers.dense(inputs=h_fc1, units=ACTIONS,
                kernel_initializer=tf.random_normal_initializer(0, stddev=0.01),
                bias_initializer=tf.constant_initializer(0.01),
                trainable=trainable)
            return qout
    ...
    def train_network(self, experience_buffer):
        ...
        while "flappy bird" != "angry bird":
            ...
            # 将数据存储到经验池中
            experience = np.reshape(
                np.array([s_t, a_t, r_t, s_t1, terminal]), [1, 5])
            ...
            if t > OBSERVE:
                # 采集样本
                train_s, train_a, train_r, train_s_, train_terminal = \
                    experience_buffer.sample(BATCH)
                target_q = []
                ...
                # 训练一次
                self.sess.run(self.q_train_op, feed_dict={
                              self.obs: train_s, self.action: train_a, self.q_target: target_q})
                # 更新旧的目标网络
                # if t%1000 == 0:
            ...
            # 每10000次迭代保存一次
            if t % 10000 == 0:
                self.save_model('models/', global_step=t)

            # if t <= OBSERVE:
            #     print("OBSERVE", t)
            # else:
            #     if t % 1 == 0:
            #         print("train, steps", t, "/epsilon", epsilon,
            #               "/action_index", a_t, "/reward", r_t)
            ...
            print("TRAIN STEP", t, "/ STATE", state,
                "/ EPSILON %.6f" % epsilon, "/ ACTION", a_t, "/ REWARD", r_t,
                "/ TERMINAL", terminal, "/ Q_MAX %e" % max(target_q))
